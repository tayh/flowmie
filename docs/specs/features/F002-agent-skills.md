...
- **Codex now wired (was deferred):** Codex is registered per-invocation via `codex -c mcp_servers.flowmie.command/args/env.*` overrides (`codex_skills_args`), verified with `codex mcp list`/`get` showing `flowmie` enabled with the baked-in env. Only OpenCode remains unwired (not installed here). The per-terminal Skills toggle UI from §7 **now exists** (see below); the `skillsEnabled` field + default-on-for-agents already existed and persisted.

- **Skills toggle UI (was deferred, now built):** the terminal node's titlebar shows a `skills on`/`skills off` pill for agent nodes only (a shell has no MCP config to strip). Clicking it calls `toggleSkills`, flipping `skillsEnabled` in the node's data. Because the MCP config is wired at spawn (`commands.rs`: `skills_enabled=false` → `skills_spawn=None` → no config), the switch changes only what the **next** spawn does — a running agent keeps the tools it launched with, and the tooltip says so ("Applies on next respawn") while a process is live. The value persists with the node and is re-read by both `addTerminal` and `respawnNode`. Verified: `tsc` clean, `vitest` 52 green, `vite build` clean; the live respawn-strips-config check is a manual step (`npm run tauri dev`).
